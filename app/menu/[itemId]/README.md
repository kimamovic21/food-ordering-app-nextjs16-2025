...
- App route/group: `/menu/[itemId]`
- Folder path: `app/menu/[itemId]`
...
- This folder talks to `/api/menu-items?_id=${itemId}`, `/api/restaurant/${item.restaurantId}`. Keep API response shapes aligned.
...
If someone asks what this folder does, say: this is the `customer` UI for `/menu/[itemId]`; it coordinates the files above, protects the edge cases listed here, and delegates server-authoritative checks to the API routes/helpers instead of trusting only the browser.
