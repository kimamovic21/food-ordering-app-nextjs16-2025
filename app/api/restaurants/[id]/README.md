...
- Source file: `app/api/restaurants/[id]/route.ts`
...
- Common response fields detected: `context`, `id`, `error`, `includeCourierReadiness`, `restaurantId`, `restaurantName`, `isAcceptingOrders`, `restaurant`, `isOpen`, `isPaused`, `pauseReason`, `orderingUnavailableReason`, `deliveryRadiusKm`, `minimumOrderAmount`, `activeOrderLimit`, `maxItemsPerOrder`, `activeKitchenOrders`, `capacityMessage`, `capacitySlotsRemaining`, `availableCouriers`, `courierReadinessDelayMinutes`, `courierReadinessMessage`, `courierReadinessTone`, `estimatedPreparationMinutes`, `estimatedDeliveryMinutes`, `estimatedTotalMinutes`, `etaDelayMinutes`, `etaMessage`, `etaTone`, `isCourierReady`, `isBusy`, `isNearCapacity`, `orderingMessage`, `totalCouriers`, `averageRating`, `ratingCount`, `details`

## How To Explain This In A Presentation

Open this file when someone asks what `/api/restaurants/[id]` does. Explain that it belongs to the public restaurant discovery/details workflow, serves `customer`, `public visitor`, validates the inputs and access rules above, then returns a stable JSON response or a clear error status.
