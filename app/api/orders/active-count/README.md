# API Route: /api/orders/active-count

> Enhanced route documentation. Keep this file aligned with the adjacent `route.ts` when behavior changes.

## Source And Ownership

- Source file: `app/api/orders/active-count/route.ts`
- Route: `/api/orders/active-count`
- HTTP methods: `GET`
...
- Line 13: `if (!userEmail) {`
- Line 19: `if (!user || user.role !== 'admin') {`
- Line 23: `if (!user.restaurantId) {`
...
- Common response fields detected: `error`, `email`, `restaurantId`, `orderStatus`, `nin`

## How To Explain This In A Presentation

Open this file when someone asks what `/api/orders/active-count` does. Explain that it belongs to the order operations workflow, serves `admin`, `restaurant admin`, validates the inputs and access rules above, then returns a stable JSON response or a clear error status.
