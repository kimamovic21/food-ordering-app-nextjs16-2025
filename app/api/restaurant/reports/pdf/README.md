# API Route: /api/restaurant/reports/pdf

> Enhanced route documentation. Keep this file aligned with the adjacent `route.ts` when behavior changes.

## Source And Ownership

- Source file: `app/api/restaurant/reports/pdf/route.ts`
- Route: `/api/restaurant/reports/pdf`
- HTTP methods: `GET`
...
- Line 21: `if (!session?.user?.email) {`
- Line 26: `if (!user || user.role !== 'admin' || !user.restaurantId) {`
- Line 33: `if (!restaurant) {`
- Line 60: `if (!report.hasActivity) {`
...
- Common response fields detected: `request`, `error`, `email`, `restaurantId`, `createdAt`, `gte`, `lte`, `period`, `label`, `start`, `end`, `restaurant`, `name`, `street`, `city`, `postalCode`, `country`, `contact`, `generatedAt`

## How To Explain This In A Presentation

Open this file when someone asks what `/api/restaurant/reports/pdf` does. Explain that it belongs to the restaurant-owner operations workflow, serves `restaurant admin`, validates the inputs and access rules above, then returns a stable JSON response or a clear error status.
