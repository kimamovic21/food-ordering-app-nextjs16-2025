...
- Source file: `app/api/statistics/route.ts`
...
- It uses `menuItem`, `notification`, `order`, `restaurant`, `supportTicket`, `user` for persistence.
...
- Line 20: `if (!(await isSuperAdmin())) {`

## Data Dependencies

- Models: `menuItem`, `notification`, `order`, `restaurant`, `supportTicket`, `user`
...
- Common response fields detected: `error`, `createdAt`, `isAvailable`, `in`, `isRead`, `totalUsers`, `totalRestaurants`, `totalAdmins`, `totalCouriers`, `totalCustomers`, `statistics`

## How To Explain This In A Presentation

Open this file when someone asks what `/api/statistics` does. Explain that it belongs to the statistics workflow, serves `super admin`, `admin`, validates the inputs and access rules above, then returns a stable JSON response or a clear error status.
