# App Folder: app/admin-dashboard/coupons/edit/[id]

> Enhanced folder documentation. Keep this file aligned with the folder workflow when behavior changes.

## Purpose

This folder owns admin-facing UI for `/admin-dashboard/coupons/edit/[id]`, including restaurant operations, platform management, and protected dashboard workflows.

## Route And Audience

- App route/group: `/admin-dashboard/coupons/edit/[id]`
- Folder path: `app/admin-dashboard/coupons/edit/[id]`
...
- This folder talks to `/api/coupons`, `/api/coupons?id=${encodeURIComponent(id)}`. Keep API response shapes aligned.
...
If someone asks what this folder does, say: this is the `admin`, `super admin` UI for `/admin-dashboard/coupons/edit/[id]`; it coordinates the files above, protects the edge cases listed here, and delegates server-authoritative checks to the API routes/helpers instead of trusting only the browser.
